...
public class SpringManager : MonoBehaviour
{
    ...
    public void Update()
    {
        ...
        if (Cursor != null)
        {
            ...
            bool isAllowed = hitInfo.transform != null && hitInfo.transform.gameObject.name.Contains("MonoObject");
...
public class WeldManager : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Cursor != null)
        {
            ...
            bool isAllowed = hitInfo.transform != null && hitInfo.transform.gameObject.name.Contains("MonoObject") && hitInfo.collider != null && hitInfo.collider.attachedRigidbody != null;
            Cursor.GetComponent<Renderer>().material.color = isAllowed ? new Color(0.392f, 0.722f, 0.820f, 0.4509804f) : new Color(0.8314f, 0.2471f, 0.1569f, 0.4509804f);
        ...
        if (editMode)
        {
            ...
            if (primaryDown)
            {
                if (canPlace && isAllowed && RefCache.HitExists)
                {
                    ...
                    NetworkManager.RegisterSpawned("balloon", BalloonOBJ);
                    HapticManager.Haptic(HapticManager.HapticType.Create);
                    canPlace = false;
                }
            }
...
public class Balloon : MonoBehaviour
{
    ...
    private void FixedUpdate()
    {
        ...
        rb.AddForce(0, -Physics.gravity.y + (float)Math.Pow(3, 4) + power, 0);
    }
}
...
public class BoneSphere : MonoBehaviour
{
    ...
    private void Start()
    {
        if (transform.childCount == 0 || transform.GetChild(0).childCount < 7) return;
