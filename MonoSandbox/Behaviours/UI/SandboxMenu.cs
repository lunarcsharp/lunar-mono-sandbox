...
namespace MonoSandbox.Behaviours.UI
{
    public class SandboxMenu : MonoBehaviour
    {
        ...
        public void Update()
        {
            ...
            UpdatePage(0, _objParent);
            UpdatePage(1, _weaponsParent);
            UpdatePage(2, _toolParent);
            UpdatePage(3, _utilsParent);
            UpdatePage(4, _funParent);
        }

        void UpdatePage(int pageIndex, GameObject pageParent)
        {
            int canvasChildIndex = pageIndex * 2;
            bool shouldShow = _currentPage == pageIndex;

            if (shouldShow && !pageParent.activeSelf)
            {
                pageParent.SetActive(true);
                _canvas.transform.GetChild(canvasChildIndex).gameObject.SetActive(true);
            }
            else if (!shouldShow && pageParent.activeSelf)
            {
                pageParent.SetActive(false);
                _canvas.transform.GetChild(canvasChildIndex).gameObject.SetActive(false);
            }
        }
